...
function draw() {
  ...
  if(keyIsPressed){
  	var l = new Letter(width/2,height/2,random(-12,12),random(-12,12),random(32),random(1,5),key,random(font));
    letters.push(l);
	song.play();
  }
...
class Letter {
  ...
  animate() {
    ...
    if (this.x > width-this.s|| this.x<0) { 
      ...
      this.yMove = 0;
      this.sizeGrowth = 0;
      this.y > height-this.s|| this.y<this.s
    }
    if (this.y > height-this.s|| this.y<this.s) {
      this.x > width-this.s|| this.x<0
      this.yMove = 0;
      this.xMove = 0;
      this.sizeGrowth = 0;
      print(this.x);
